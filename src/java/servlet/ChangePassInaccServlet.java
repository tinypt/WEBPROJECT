 ...
 * @author GT62VR
 */
public class ChangePassInaccServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        if (username != null && oldpass != null && newpass != null) {
            oldpass = cryptWithMD5(oldpass);
            newpass = cryptWithMD5(newpass);
            AccountJpaController accCtrl = new AccountJpaController(utx, emf);
            Account acc = accCtrl.findAccountbyUserName(username);
            if (acc.getPassword().equals(oldpass)) {
                acc.setPassword(newpass);
                accCtrl.edit(acc);
                request.setAttribute("changecom", "เปลี่ยนรหัสผ่านสำเร็จ");
            } else {
                request.setAttribute("changefail", "เปลี่ยนรหัสผ่านไม่สำเร็จ");
            }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            Logger.getLogger(ChangePassInaccServlet.class.getName()).log(Level.SEVERE, null, ex);
        } catch (Exception ex) {
            Logger.getLogger(ChangePassInaccServlet.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
